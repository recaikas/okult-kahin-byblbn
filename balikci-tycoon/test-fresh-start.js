...
(async () => {
  ...
  const ok = !kayit && bas.acik >= 1 && bas.sellable.length >= 1 && son.ag[0] > 0 && !errs.length;
  console.log(JSON.stringify({ kayitYok: !kayit, bas, son, FRESH_OK: ok, errs: [...new Set(errs)] }, null, 1));
  process.exit(ok ? 0 : 1);
})();
